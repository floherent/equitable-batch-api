...
class ChunkGenerator:
    def __init__(self, indir, blocksize=None, chunksize=None):
        ...
        logger.info(f'Generator initialized with {len(self.files)} file(s)')
    ...
    def get_next(self):
        ...
        while len(chunks) < self.num_chunk and not self.done:
            ...
            num_chunks_available = min(
                self.num_chunk - len(chunks),
                (remaining_records + self.chunk_size - 1) // self.chunk_size
            )
...
class ChunkProcessor:
    ...
    def save(self):
        ...
        filepath = os.path.join(self.outdir, f'{self.chunk_id}_output.csv')
